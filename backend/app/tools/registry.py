...
BUILTIN_TOOL_SPECS: List[Dict[str, Any]] = [
    ...
    },
    {
        "name": "object_removal",
        "category": "generative",
        "purpose": "Remove unwanted objects from the image using specialized LaMa inpainting.",
        "description": "Uses LaMa (Large Mask Inpainting) — a CNN with Fourier Convolutions purpose-built for filling large masked regions. Delivers superior structural coherence vs diffusion-based inpainting.",
        "strengths": ["Specialized LaMa model", "Handles large masks", "Structural coherence", "Clean removal"],
        "weaknesses": ["GPU recommended", "May need multiple attempts on complex backgrounds"],
        "best_for": ["Removing objects", "Removing clutter", "Cleaning scenes", "Large region removal"],
        "avoid_for": ["Faces (use person_removal instead)", "Text/logos (garbled)"],
        "required_inputs": ["image", "mask_or_target"],
        "outputs": ["edited_image"],
        "compatible_tools": ["segment", "generative_fill", "healing", "clone", "person_removal"],
        "parameters": {
            "target": {"type": "string", "default": "", "description": "Object description for automatic SAM segmentation"},
            "prompt": {"type": "string", "default": "", "description": "Optional fill description"}
        },
        "gpu_cost": "medium",
        "latency": "medium",
    },
    {
        "name": "person_removal",
        "category": "generative",
        "purpose": "Remove people from the image using specialized person segmentation + LaMa inpainting.",
        "description": "Uses u2net_human_seg for precise person mask generation combined with LaMa (Large Mask Inpainting) for seamless background filling. Purpose-built for removing people from scenes.",
        "strengths": ["Specialized person segmentation", "Clean person removal", "Handles multiple people"],
        "weaknesses": ["GPU recommended", "May struggle with complex occlusions"],
        "best_for": ["Removing people", "Cleaning tourist photos", "Emptying scenes", "Privacy masking"],
        "avoid_for": ["Small objects (use object_removal)", "Very large crowds (50%+ of image)"],
        "required_inputs": ["image"],
        "outputs": ["edited_image", "person_mask"],
        "compatible_tools": ["object_removal", "segment", "generative_fill", "inpainting"],
        "parameters": {
            "target": {"type": "string", "default": "", "description": "Optional person description (e.g. 'the man in red')"},
            "refine_faces": {"type": "bool", "default": True, "description": "Refine mask around face regions"}
        },
        "gpu_cost": "medium",
        "latency": "medium",
    },
